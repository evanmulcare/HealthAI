...
const HeartDiseaseModal = ({
  setShowHeartDiseaseModal,
  ...
  const sendReportRequest = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://healthaibackendtester.onrender.com/predict-heart-disease",
        {
          method: "POST",
          headers: {
              ...
              <div>
                {predictionResult ? (
                  ...
                  <>
                    <Confetti />
                    <h1 className="font-semibold text-gray-800 text-2xl text-center">
                      Good News, Heart Disease Unlikely
                    </h1>
                    <p className="font-semibold text-gray-600 text-xl text-center">
                      Based on the available datasets it is likely that this
