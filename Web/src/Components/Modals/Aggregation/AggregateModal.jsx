...
const AggregateModal = ({
  ...
  const fetchData = async (type) => {
    try {
      const response = await fetch(
        `https://healthaibackendtester.onrender.com/download-${type}-data-csv`
      );
  ...
  const reports = useSelector((state) =>
    selectReportsByPatientIds(state, selectedPatientsIds, type)
  );
        ...
        <div className="flex justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-800">
              {t("aggregatemodal.titleone")} {title}{" "}
              {t("aggregatemodal.titletwo")}
            </h1>
          ...
          <div className="flex items-center">
            <button
              className={`px-4 py-2 rounded-md border-1 inline-flex ${
