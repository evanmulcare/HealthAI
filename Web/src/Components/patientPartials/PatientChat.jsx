...
const PatientChat = ({ reciever }) => {
  ...
  const selectCommentsByParentId = (state, recieverId) => {
    return state.comments.comments.filter(
      (comment) => comment.reciever == recieverId
    );
  };

  const comments = useSelector((state) =>
    selectCommentsByParentId(state, reciever?.docId)
  );
  ...
  return (
    <div className="overflow-y-hidden">
          ...
          <div className="flex flex-col">
            {comments.length === 0 ? (
              ...
              comments.map((comment) =>
                renderComment(comment, currentUser, userData)
              )
            )}
          </div>
        </div>
      ...
      <div className="fixed bottom-0 p-3 inline-flex items-center border-t w-full bg-gray-50">
        ...
        <button
          className="text-blue-500 hover:text-blue-700 m-3 md:m-5"
