...
const renderComment = (comment, currentUser, userData) => {
  ...
  const commentDate =
    comment?.created && comment?.created.toDate
  ...
  return (
    <div
      key={comment.id}
      className={`${isCurrentUser ? "self-end" : "self-start"} mb-2`}
    >
      <div className="flex justify-start">
        ...
        <div>
          <div className={`text-gray-700 text-xs ${isRightAligned}`}>
            {isCurrentUser
              ...
              : `${commentUser?.firstName} ${commentUser?.lastName}  ${
                  comment?.created ? formattedDate : "N/A"
                }`}
          </div>
          <div
            className={`${
          ...
          >
            <p className="max-w-xs overflow-hidden text-base break-words">
              {comment?.comment}
            </p>
          </div>
        </div>
